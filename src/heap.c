...
static void *map_in_pages(int page_count) {
  ...
  void *addr = mmap(NULL, page_count * PAGE_SIZE, PROT_READ | PROT_WRITE,
...
void heap_create(heap *h, int initial_size, int (*comp_func)(void *, void *)) {
  ...
  h->allocated_pages = initial_size / ENTRIES_PER_PAGE +
                       ((initial_size % ENTRIES_PER_PAGE > 0) ? 1 : 0);
...
int heap_delmin(heap *h, void **key, void **value) {
  ...
  *value = current->value;
  ...
  if (h->active_entries > 0) {
    ...
    while (left_child_index = LEFT_CHILD(current_index),
           left_child_index < entries) {
      // Load the left child
  ...
  int used_pages =
      entries / ENTRIES_PER_PAGE + ((entries % ENTRIES_PER_PAGE > 0) ? 1 : 0);

  // Allow one empty page, but not two
  if (h->allocated_pages / 2 > used_pages + 1 &&
      h->allocated_pages / 2 >= h->minimum_pages) {
    // Get the new number of entries we need
    int new_size = h->allocated_pages / 2;
