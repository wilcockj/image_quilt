...
typedef struct heap {
  ...
  int minimum_pages;   // The minimum number of pages to maintain, based on the
                       // initial cap.
 ...
 * @param initial_size What should the initial size of the heap be. If <= 0,
 * then it will be set to the minimum permissable size, of 1 page (512 entries
 * on 32bit system with 4K pages).
 * @param comp_func A pointer to a function that can be used to compare the
 * keys. If NULL, it will be set to a function which treats keys as signed ints.
 * This function must take two keys, given as pointers and return an int. It
 * should return -1 if key 1 is smaller, 0 if they are equal, and 1 if key 2 is
 * smaller.
 */
 ...
 * @return 1 if the minimum element exists and is set, 0 if there are no
 * elements.
 */
 ...
 * @return 1if the minimum element exists and is deleted, 0 if there are no
 * elements.
 */
 ...
 * @param func The function to call on each entry. Should take a void* key and
 * value.
 */
